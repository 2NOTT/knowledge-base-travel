...
class NodePDFToMD(BaseNode):
    ...
    def _step_2_upload_and_poll(self, pdf_path_obj: Path):
        ...
        # 2.2发送请求
        response = requests.post(url, headers=header, json=data, timeout=(10, 30))
        if response.status_code != 200:
            raise PdfConversionError("获取上传连接失败 响应状态：{}，响应结果{}".format(response.status_code, response))
        ...
        with open(pdf_path_obj, 'rb') as f:
            res_upload = requests.put(urls[0], data=f, timeout=(10, 300))
            if res_upload.status_code != 200:
                raise PdfConversionError(f"{urls[0]}文件上传失败")

            self.logger.info(f"上传文件成功!")
        ...
        while True:
            ...
            if elapsed_time > timeout_seconds:
                raise TimeoutError(f"[任务轮询]超时，已消耗的时间{int(elapsed_time)}s,batch_id:{batch_id}")
            ...
            try:
                ...
                self.logger.warning(f"一次信息:{str(e)}")
            ...
            if data_state == "done":
                self.log_step(
                    step_name="任务轮询",
                    message=f"解析完成，总耗时：{int(elapsed_time)}s,batch_id:{batch_id}"
                )
                ...
                self.log_step(
                    step_name="任务轮询",
                    message=f"处理中......：已耗时：{int(elapsed_time)}s,状态：{data_state},"
                            f"batch_id:{batch_id}"
                )
                time.sleep(poll_interval)
...
if __name__ == "__main__":
    setup_logging()
    logging.getLogger().info("请通过导入 API 调用 MinerU 解析旅游 PDF")
