...
class NodeMDImg(BaseNode):
    ...
    def _step_2_scan_images(self, md_content: str, images_dir: Path) -> List[Tuple[str, str, Tuple[str, str]]]:
        ...
        for image_file in os.listdir(images_dir):
            ...
            # 3.查找这个图片在md文档当中的上下文
            context = self._find_image_in_md(
                md_content,
                image_file,
                context_len=self.config.img_content_length,
            )
    ...
    def _step_3_generate_summaries(self, doc_stem: str, target_images: List[Tuple[str, str, Tuple[str, str]]]) -> Dict[str, str]:
        ...
        for image_file, img_path, context in target_images:
            # 3.1限速
            self._apply_api_rate_limit(request_deque, 3, 10)

            # 3.2向模型发送请求
            summaries[image_file] = self._summarize_image(img_path, root_folder=doc_stem, image_content=context)

        return summaries

    def _step_4_upload_and_replace(self,doc_stem: str,target_images: List[Tuple[str, str, Tuple[str, str]]],summaries: Dict[str, str],md_content: str) -> str:
        """
        ...
        # 3.清理已有目录
        self.clean_minio_dir(minio_client, upload_dir)
    ...
    def _upload_to_minio(self, minio_client: Minio, local_path: str, object_name: str) -> str | None:
        ...
        try:
            ...
            bucket_name = minio_config.bucket_name.strip("/")
            base_url=f"http://{endpoint}/{bucket_name}/{object_name}"
            return base_url
        except:
            self.logger.error(f"图片上传失败{local_path}")

    def clean_minio_dir(self, minio_client: Minio, update_dir: str) -> None:

        try:
            #1.获取将要被删除的图片列表
            objets_to_delete = minio_client.list_objects(minio_config.bucket_name, update_dir, recursive=True)

            delete_list=[DeleteObject(obj.object_name) for obj in objets_to_delete]
    ...
    def _apply_api_rate_limit(self,request_times: Deque[float],max_requests: int,window_seconds: int = 5) -> None:
        """
        ...
        self.logger.info(f"{self.name}请求成功，当前{window_seconds}秒窗口内请求数：{len(request_times)}")

    def _find_image_in_md(self,md_content: str,  image_file: str,  context_len=100) -> Tuple[str, str]:

        # re.escape:要将参数中的特殊符号进行转义
        # 1.定义正则表达式
        pattern = re.compile(r"!\[.*?\]\(.*?" + re.escape(image_file) + r".*?\)")
...
if __name__ == "__main__":
    setup_logging()
    logging.getLogger().info("请通过导入 API 处理旅游 Markdown 图片")
