...
class NodeRrf(NodeBase):
    ...
    将多路本地召回的结果（普通向量、HyDE）进行加权融合排序。

    联网结果保留到 ``node_rerank`` 合并，因为网页结果没有 Milvus
    ``chunk_id``，不能直接和本地切片共享去重键。
    """
    ...
    def process(self, state: QueryGraphState) -> QueryGraphState:
        ...
        embedding_search_list = self._normalise_hits(embedding_chunks_list)
        hyde_embedding_search_list = self._normalise_hits(hyde_embedding_chunks_list)
    ...
    @staticmethod
    def _normalise_hits(hits: list) -> List[Dict]:
        """把 Milvus 命中统一成带稳定 ``chunk_id`` 的实体字典。"""
        ...
        for hit in hits or []:
            ...
            entity = dict(entity) if isinstance(entity, dict) else dict(hit)
            if entity.get("chunk_id") is None:
                hit_id = hit.get("id", hit.get("pk"))
                if hit_id is not None:
                    entity["chunk_id"] = hit_id
            if entity.get("chunk_id") is None:
                continue
