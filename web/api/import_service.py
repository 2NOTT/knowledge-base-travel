...
app = FastAPI(
    title="知识库-导入API",
    description="此文档是知识库导入流程的API接口说明"
)
...
async def upload_files(background_tasks: BackgroundTasks, files: List[UploadFile] = File(...)):
    ...
    data_based_root_dir = os.getenv("DATA_BASED_ROOT_DIR", "./data")
    ...
    for file in files:
        ...
        minio_object_name = f"pdf_files/{datetime.now().strftime('%Y%m%d')}/{safe_filename}"
        try:
            # 获取MinIO客户端实例
