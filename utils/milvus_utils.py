...
def build_in_expr(field: str, values: list[str] | None) -> str | None:
    """构造安全的 Milvus ``in`` 过滤表达式；没有值时返回 None。"""

    clean_values = [str(value).strip() for value in (values or []) if str(value).strip()]
    if not clean_values:
        return None
    encoded_values = ", ".join(json.dumps(value, ensure_ascii=False) for value in dict.fromkeys(clean_values))
    return f"{field} in [{encoded_values}]"


def build_travel_expr(cities: list[str] | None = None, entities: list[str] | None = None) -> str | None:
    """按城市或旅游主体构造可选过滤条件；两者都没有时执行全库检索。"""

    expressions = []
    city_expr = build_in_expr("city", cities)
    entity_expr = build_in_expr("entity_name", entities)
    if city_expr:
        expressions.append(city_expr)
    if entity_expr:
        expressions.append(entity_expr)
    return " or ".join(f"({expression})" for expression in expressions) or None
...
def hybrid_search(client, collection_name, reqs, ranker_weights=(0.5, 0.5), norm_score=True, limit=5,
    ...
    :param output_fields: 需要返回的字段列表，默认返回旅游切片字段
    ...
    try:
        ...
        if output_fields is None:
            output_fields = ["chunk_id", "content", "title", "file_title", "city", "entity_name"]
